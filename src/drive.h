...
/**
 * @brief Function made for finding alternative path by seting the edges in the best path to visited
 ...
 * @return std::vector<pair<int,int>> Vector with edges to avoid
 */
inline std::vector<std::pair<int, int>> edglimit(int source, int dest, std::vector<int> &path)
{
    std::vector<std::pair<int, int>> limit;
    for (int i = 0; i < path.size() - 1; i++)
    {
        limit.emplace_back(path[i], path[i + 1]);
    }
    return limit;
}
/**
...
/**
 * @brief Compare two pairs of double and int
 * @details Complexity O(1)
 * @param d1 First pair
 * @param d2 Second pair
 * @return bool - true if d1 is greater than d2, false otherwise
 */
 ...
 * @return std::pair<std::vector<int>,double> - path and distance
 */
...
inline singleMode noRestriction(int source, int dest, Graph graph, int mode)
{
    ...
    std::vector<std::pair<int, int>> edglim = edglimit(source, dest, result.bestpath);
    unsetNodesAndEdges(graph, mode);
    someNodesVisited(graph, lim);
    someEdgesVisited(graph, edglim);
    algorithm(source, graph, mode);
    path = getPath(source, dest, graph, mode);
...
{
    singleMode result;
    if (graph.findNodeIndex(source) == -1 || graph.findNodeIndex(dest) == -1 || graph.findNodeIndex(include) == -1)
        return result;
    std::vector<Node *> Nodes = graph.getNodes();
    std::pair<std::vector<int>, double> path;
    unsetNodesAndEdges(graph, mode);
 ...
 * @param sourc The starting node ID
 * @param dest The destination node ID
 * @param graph Graph object - with nodes
...
/**
 * @brief Get the parking nodes from the graph
 * @details Complexity O(V)
 * @param graph Graph object
 * @return std::unordered_set<int> - set of parking node IDs
 */
...
/**
 * @brief Compare two WalkDrive objects
 * @details Complexity O(1)
 * @param d1 First WalkDrive object
 * @param d2 Second WalkDrive object
 * @return bool - true if d1 is greater than d2, false otherwise
 */
...
/**
 * @brief Find the shortest path between two nodes with parking
 * @details Complexity O((V + E) log(V)+V²)
 * @param source The starting node ID
 * @param dest The destination node ID
 * @param graph Graph object - with nodes
 ...
 * @param al Vector of WalkDrive objects
 * @return WalkDrive - best path and distance
 */
...
inline WalkDrive walkingDriving(int source, int dest, Graph &graph, std::unordered_set<int> &limit, std::vector<std::pair<int, int>> &edgesToAvoid, double maxWalkingTime, std::vector<WalkDrive> &al)
{
    ...
    for (WalkDrive a : al)
    {
        if (a.walkingRoute.second <= maxWalkingTime)
        {
            result = a;
            return result;
        }
